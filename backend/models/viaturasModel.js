...
module.exports = {
  ...
  saida: ({ viatura, motorista, nip, destino, cabo }) => {
    const tx = db.transaction(() => {
      ...
      const r = db.prepare(`
        ...
        VALUES (?,?,?,?,?,?,?, 'em_uso')
      `).run(viatura.id, viatura.prefixo, motorista, nip || null, destino, km, cabo || null);
  ...
  retorno: ({ viatura_id, km_retorno, autonomia, cabo }) => {
    const tx = db.transaction(() => {
      ...
      const rodado = (km_retorno || 0) - (reg.km_saida || 0);
      db.prepare(`
        ...
        WHERE id=?
      `).run(km_retorno, rodado, autonomia || null, cabo || null, reg.id);
